...
export fn worm_cbor_encode(record: ?*constants.Record, buffer: [*c]u8, len: [*c]usize) i32 {
    ...
    // Note: encode_record returns fixed array [512]u8 (no allocation), only memcpy
...
// CALLER RESPONSIBILITY: Must call worm_free on returned Record to avoid leaks
...
// C export: worm_free_record(record: *Record) -> void
// Frees a Record allocated by worm_create_record or worm_cbor_decode
export fn worm_free_record(record: ?*anyopaque) void {
    if (record == null) return;
    gpa.allocator().destroy(@as(*constants.Record, @ptrCast(@alignCast(record))));
}

// C export: worm_free_writer(writer: *Writer) -> void
// Frees a Writer and its associated Storage
export fn worm_free_writer(writer: ?*anyopaque) void {
    if (writer == null) return;
    const w = @as(*Writer, @ptrCast(@alignCast(writer)));
    if (w.storage != null) {
        w.storage.?.deinit();
    }
    gpa.allocator().destroy(w);
}

// C export: worm_free(obj: *anyopaque) -> void
// DEPRECATED: Use worm_free_record or worm_free_writer instead
// Attempts generic destruction (unsafe, may crash if type is unknown)
export fn worm_free(obj: ?*anyopaque) void {
    if (obj == null) return;
    gpa.allocator().destroy(@as(*Writer, @ptrCast(@alignCast(obj))));
}

// C export: worm_shutdown() -> void
// Cleanup: Free writers HashMap and shutdown allocator
export fn worm_shutdown() void {
    if (initialized) {
        var iter = writers.valueIterator();
        while (iter.next()) |writer_ptr| {
            if (writer_ptr.*.storage != null) {
                writer_ptr.*.storage.?.deinit();
            }
            gpa.allocator().destroy(writer_ptr.*);
        }
        writers.deinit();
        initialized = false;
    }
